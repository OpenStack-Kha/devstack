...
if "key" in ENABLED_SERVICES:
    screen_it("key","cd /opt/stack/keystone && bin/keystone --config-file /opt/stack/keystone/etc/keystone.conf -d")
...
# launch the horizon and wait for it to answer before continuing
if "n-api" in ENABLED_SERVICES:
    screen_it("horizon","cd /opt/stack/horizon/openstack-dashboard/ && /usr/bin/python ./manage.py runserver 0.0.0.0:8181")
    print "Waiting for horizon to start..."
    try:
        strt_results = sp.check_output('wget -q -O- http://127.0.0.1:8181', shell= True)
    except sp.CalledProcessError as exc:
        if exc.returncode != 4:
            raise
        else:
            strt_results = ''
    if  strt_results == '':
        print "horizon did not start"
    else:
        print "horizon has been started successful"
...
if "n-vnc" in ENABLED_SERVICES:
    screen_it("n-vnc","cd /opt/stack/noVNC && ./utils/nova-wsproxy.py --flagfile /opt/stack/nova/bin/nova.conf --web . 6080")
if "n-cauth" in ENABLED_SERVICES:
    screen_it("n-cauth","cd /opt/stack/nova/ && ./bin/nova-consoleauth")
if "n-xvnc" in ENABLED_SERVICES:
    screen_it("n-xvnc","cd /opt/stack/nova/ && ./bin/nova-xvpvncproxy --flagfile /etc/nova/nova.conf")
if "n-novnc" in ENABLED_SERVICES:
    screen_it("n-novnc","cd /opt/stack/noVNC/ && ./utils/nova-novncproxy --flagfile /etc/nova/nova.conf")
...
if "swift" in ENABLED_SERVICES:
 sp.call("mount /opt/stack/swift/data/drives/images/swift.img /opt/stack/swift/data/drives/sdb1 -o loop; cd /opt/stack/nova && /usr/bin/swift-init all start", shell=True)
    #screen_it("swift","sudo mount /opt/stack/swift/data/drives/images/swift.img /opt/stack/swift/data/drives/sdb1 -o loop; #cd /opt/stack/nova && swift-init all start")

#if "horizon" in ENABLED_SERVICES:
#    screen_it("horizon","cd /opt/stack/horizon && sudo tail -f /var/log/httpd/error.log")
